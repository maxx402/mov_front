...
export default function ChannelDetailPage({ params }: Props) {
  ...
  const channelCover = searchParams.get("cover") ?? "";
  const channelDesc = searchParams.get("desc") ?? "";
  ...
  const loadMovies = useCallback(async (page: number) => {
    const result = await movieRepo.getMoviesByChannel({ channelId: id, page, pageSize: 20 });
    return result;
  }, [movieRepo, id]);
    ...
    <div className="flex h-screen flex-col bg-scaffold">
      ...
      {isLoading ? (
        ...
        <div className="relative flex-1 overflow-hidden">
          ...
          >
            {/* Transparent spacer (reveals cover behind) */}
            {channelCover && <div style={{ height: 126 }} />}
