...
import { getWatchProgressDisplayText } from "@/domain/entities/watch-history";
...
function WatchHistoryContent() {
  ...
  const historyRepo = useMemo(() => container.get<IHistoryRepository>(DI_KEYS.HistoryRepository), [container]);
  ...
  const loadItems = useCallback(async (page: number) => {
    return historyRepo.getWatchHistories({ page, pageSize: 20 });
  }, [historyRepo]);
  ...
  const handleDelete = async () => {
    ...
    await historyRepo.removeWatchHistories(movieIds);
  ...
  return (
    <div className="flex h-screen flex-col bg-scaffold">
      ...
      {isLoading ? (
        ...
        >
          {items.map((item) => {
            const progressText = getWatchProgressDisplayText(item);
            return (
              <button
                key={item.id}
                onClick={() => {
                ...
                className="mb-3 flex w-full items-center text-left"
                style={{ gap: 10 }}
              >
                {/* Selection indicator */}
                {isEditMode && (
                  <div className="shrink-0">
                    <div
                      style={{
                        ...
                        height: 22,
                        borderRadius: "50%",
                        border: selectedIds.has(item.movieId) ? "none" : "1.5px solid rgba(255,255,255,0.8)",
                        background: selectedIds.has(item.movieId) ? "#FFE5B4" : "transparent",
                      }}
                    />
                  </div>
                )}

                {/* Cover (horizontal 100x70) */}
                <div className="shrink-0">
                  <OptimizedImage
                    src={item.movieCover}
                    alt={item.movieTitle}
                ...
                {/* Info */}
                <div className="flex min-w-0 flex-1 flex-col" style={{ height: 70, justifyContent: "center" }}>
                  <span className="truncate" style={{ fontSize: 14, fontWeight: 500, color: "#FFFFFF" }}>
                    {item.movieTitle}
                  </span>
                  <span className="mt-1 truncate" style={{ fontSize: 12, color: "rgba(255,255,255,0.6)" }}>
                    {progressText}
                  </span>
                </div>
              </button>
